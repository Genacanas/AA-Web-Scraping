...
url = "https://goecm.com/products/2002-07-ford-f350-fuse-box-module"
...
# try:
tabla = soup.select_one("div.rte table")             #NO ENCUENTRA LA TABLA YA QUE SE CARGA DINAMICAMENTE
filas = tabla.select("tr")

for fila in filas:
    columnas = fila.select("td")
    if len(columnas) == 2:
        key = columnas[0].get_text(strip=True)
        value = ", ".join([line.strip() for line in columnas[1].stripped_strings])
        ...
        match key:
            ...
            case "Part Number[s]":
                part_number = value
            case "Part Type":
                part_type = value.split()[-1]
            case "Engine Size":
                engine_size = value
            case "Condition":
                condition = value
            case "Replacement Number":
                replacement_number = value
            case "Hardware Number[s]":
                hardware_number = value.replace("/", ", ")
...
# Buscar texto que contenga 'Interchangeable' y 'part number'
referencia = soup.find(string=re.compile(r'Interchangeable.*part number', re.IGNORECASE))

if referencia:
    tabla_objetivo = referencia.find_next("div", class_="table-wrapper")
    if tabla_objetivo:
        celdas = tabla_objetivo.select("td")
        table_part_numbers = [celda.get_text(strip=True) for celda in celdas]
        print("Part Numbers encontrados:", table_part_numbers)
    else:
...
new_row = {
    "Heading SKU": sku,                                     #1
    "Part Number[s]": part_number,                          #2
    "Hardware Number": hardware_number,                     #3
    "Starting Year": starting_year,                         #4
    "Ending Year": ending_year,                             #5
    "Make": make,                                           #6
    "Model": model,                                         #7
    "Price": price,                                         #8
    "Stock": stock,                                         #9
    "Part Type": part_type,                                 #10
    "Engine Size": engine_size,                             #11
    "Condition": condition,                                 #12
    "Replacement Number": replacement_number,               #13
    "Description": description,                             #14
    "Interchangable Part Numbers": table_part_numbers       #15
}
